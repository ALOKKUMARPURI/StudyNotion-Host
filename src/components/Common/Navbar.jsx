...
function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
  <div className="md:hidden bg-richblack-800 px-6 py-4">
    <ul className="flex flex-col gap-4 text-richblack-25">
      {NavbarLinks.map((link, index) => (
        <li key={index}>
          {link.title === "Catalog" ? (
            <>
              <p className="font-semibold text-yellow-50">Catalog</p>
              {loading ? (
                  ...
                  .map((subLink, i) => (
                    <Link
                      ...
                      to={`/catalog/${subLink.name
                        ...
                        .join("-")
                        .toLowerCase()}`}
                      className="block pl-4 text-sm text-richblack-100 hover:text-yellow-100"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      {subLink.name}
                    </Link>
    ...
    {/* Cart Icon */}
    {user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
      <Link to="/dashboard/cart" onClick={() => setIsMobileMenuOpen(false)} className="relative mt-6 block w-fit">
        <AiOutlineShoppingCart className="text-2xl text-richblack-100" />
        {totalItems > 0 && (
      ...
      {!token && (
        <>
          <Link to="/login" onClick={() => setIsMobileMenuOpen(false)}>
            <button className="w-full rounded-md border border-richblack-700 bg-richblack-900 px-4 py-2 text-richblack-100">
              Log in
            </button>
          </Link>
